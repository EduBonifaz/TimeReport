...
try:
	WebDriverWait(driver, 4).until(expected_conditions.presence_of_element_located((By.XPATH,'//p[strong=" Día no imputable "]')))
	print("Dia no imputable")
	...
	if os.path.isfile("./Report.xlsx"):
		...
		if ReportDf[['Proyecto', 'Tarea']].equals(ReportTareasDf[['Proyecto', 'Tarea']]):
			if HorasDf.loc[0][0] == "8:00 Horas":
				...
				try:
					WebDriverWait(driver, 4).until(expected_conditions.element_to_be_clickable((By.XPATH,f'//*/c-wiz[div/div/div/div/div/div[@data-tooltip="Google Drive Folder: {year}"]]')))
				except:
				...
				try:
					WebDriverWait(driver, 4).until(expected_conditions.element_to_be_clickable((By.XPATH,f'//*/c-wiz[div/div/div/div/div/div[@data-tooltip="Google Drive Folder: {month}-{month_str}"]]')))
				except:
					WebDriverWait(driver, 30).until(expected_conditions.element_to_be_clickable((By.XPATH,r'//button[@guidedhelpid="new_menu_button"]')))
					driver.find_element(By.XPATH,r'//button[@guidedhelpid="new_menu_button"]').click()
					WebDriverWait(driver, 30).until(expected_conditions.element_to_be_clickable((By.XPATH,r'//div[div/span/div[@data-tooltip="File upload"]]')))
